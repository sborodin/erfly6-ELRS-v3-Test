...
void putsAttIdxTelemItems( uint8_t x, uint8_t y, uint8_t index, uint8_t attr )
{
	...
	//	if ( g_model.protocol == PROT_AFHDS2A )
...
{
	lcd_putsAttIdx( x, y, Str_TXeq, ( g_model.protocol == PROTO_AFHDS2A ), 0 ) ;
}
  ...
  if (sub < 6) {
    ...
    } else
    {
      ...
      if (sub > 11) {
        y = FH;
        subN = 12;
        lcd_puts_Pleft(y, PSTR("Vario"
                               "\037""Source"
                               "\037""Switch"));
        uint8_t vsource = g_model.varioData.varioSource;
        g_model.varioData.varioSource = checkIndexed(y + FH, PSTR(FWx15 "\005" STR_VSPD_A2), vsource, (sub == subN));
        y += FH;
        subN += 1;
        if (sub == subN) {
          attr = InverseBlink;
        }
        g_model.varioData.swtch = edit_dr_switch(15 * FW, y + FH, g_model.varioData.swtch, attr, attr ? EDIT_DR_SWITCH_EDIT : 0);
      }
    #endif 
...
void menuProcAdjust(uint8_t event)
{
	...
	EditType = EE_MODEL ;
	static MState2 mstate2;
 	mstate2.check_columns(event, NUM_GVAR_ADJUST-1 ) ;
        ...
        if (k <= numSafety) {
          if (j == 0) {
...
#else
            sd->opt.ss.mode = checkIndexed(y, PSTR(FWx5 "\003"
                                                        "\001SAVX"),
                sd->opt.ss.mode, active);
#endif
          } else if (j == 1) {
            int8_t max = MaxSwitchIndex;
            ...
            if (sd->opt.ss.swtch > MAX_DRSWITCH) {
              lcd_putsAttIdx(11 * FW + 3, y, PSTR(STR_V_OPT1), sd->opt.ss.swtch - MAX_DRSWITCH - 1, attr);
            } else {
#endif
            ...
            if (sd->opt.ss.mode == 1) {
              ...
              lcd_putsAttIdx(15 * FW + 3, y, Str_Sounds, sd->opt.ss.val, attr);
...
#ifdef TELEMETRY
void dispInVExVDbl( uint8_t y )
{
      lcd_puts_Pleft( y, PSTR("InV"));
      putsTelemValue( 3*FW, y-FH, (int16_t)AFHDS2A_tel_data[FST_IDX_INTV], FST_IDX_INTV, LEFT|NO_UNIT);
    ...
    //  int32_t value;
      {
        ...
        if (tview == TEL_PAGE_1) {
                ...
                if (FuelPer < g_model.FuelMin)
                  FuelPer = g_model.FuelMin;
                FuelPer = ((FuelPer - g_model.FuelMin) * 100) / (g_model.FuelMax - g_model.FuelMin);
                lcd_hbar(25, 9, 80, 6, (uint8_t)FuelPer);
                lcd_outdez(127 - FW, 1 * FH, FuelPer);
                lcd_putc(Lcd_lastPos, 1 * FH, '%');
 

            lcd_puts_Pleft( 5 * FH, PSTR("Tmp"));
            putsTelemValue( 3*FW, 4 * FH, (int16_t)AFHDS2A_tel_data[FST_IDX_TEM], FST_IDX_TEM, DBLSIZE|LEFT|NO_UNIT);
            lcd_puts_P(11*FW-2, 5*FH, PSTR("RPM"));
            putsTelemValue( 14*FW - 2, 4*FH, (int16_t)AFHDS2A_tel_data[FST_IDX_RPM], FST_IDX_RPM, DBLSIZE|LEFT|NO_UNIT);
...
//            lcd_puts_Pleft(2 * FH, PSTR(STR_12_RPM));
              lcd_puts_Pleft(2 * FH, PSTR("Telemetry page-3"));
              lcd_puts_Pleft(3 * FH, PSTR("in the development"));
              lcd_puts_Pleft(4 * FH, PSTR("process..."));
...
//                lcd_outdezAtt(11 * FW, 7*FH, FrskyHubData[FR_TXRSI_COPY], LEFT);
              ...
              lcd_puts_Pleft(2 * FH, PSTR("Telemetry page-4"));
              lcd_puts_Pleft(3 * FH, PSTR("in the development"));
              lcd_puts_Pleft(4 * FH, PSTR("process..."));
        }
...
void menuProcVoiceAlarm(uint8_t event) {
  ...
  static MState2 mstate2;
#ifdef V2
  mstate2.check_columns(event, NUM_VOICE_ALARMS + EXTRA_VOICE_SW - 1);
#else
...
#ifdef V2
				g_model.useCustomStickNames = checkIndexed( y, PSTR(FWx16"\002\005   NoRadioModel"), g_model.useCustomStickNames, (sub==subN) ) ;
#else
...
#ifdef V2
				g_model.anaVolume = checkIndexed( y, PSTR(FWx17"\012\003---P1 P2 P3 GV1GV2GV3GV4GV5GV6GV7"), tvol, (sub==subN) ) ;
#else
